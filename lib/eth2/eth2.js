...
class Eth extends EventEmitter {
    ...
    init (options, callback) {
        ...
        try {
            ...
            this.connect('web3', options.web3url, options.address, options.password, abiArray, options.updateTimeout,options.utcFile);
            this.connect('infura', options.infuraUrl, options.address, options.password, abiArray, options.updateTimeout,options.utcFile);
    ...
    async connect (name, url, address, password, abiArray, updateTimeout,utcFile) {
        const options = {
            url,
            address,
            password,
        ...
        await client.loadWallet(options);
    }
