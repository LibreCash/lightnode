...
class MasterNode extends EventEmitter {
    ...
    async nodeExchangeOn (node) {
        var client = node.client;
        await client.nodeExchangeOnOff('on');
    }

    async nodeExchangeOff (node) {
        var client = node.client;
        await client.nodeExchangeOnOff('off');
    }
        ...
        for (var i in this.nodes) {
            ...
            if (node.hasNewData()) {
                ...
                if (!res.err) {
                    ...
                    nodeError(node, 'NODE_TICKER_DELTA_OVERFLOW', 'Node error: delta > deltaMax');
                }
            }
        }
