...
class MasterNode extends EventEmitter {
    ...
    async init(options, resolve) {
        ...
        notifications.push('MASTERNODE_STARTED', this.id);
        
        eth.init(this.options.smartContract);
    ...
    updateState () {
        // todo save to db
        db.updateMasterNodeState(this.state);
    ...
    async stop () {
        if (this.state.running) {
            ...
            // todo: update state running

            while (this.state.lightNodesTotal > 0) {
                this.removeNode(this.node[0]);
            }
        }
    }
...
//        this.onNodeUpdateRates(node, rates);
    ...
    onNodeConnectionError (node) {
        ...
        this.notifications.notify('error', 'NODE_UNABLE_TO_CONNECT', {nodeId: node.id});
    }
    ...
    onNodeConnected (node) {
        ...
        this.notifications.notify('info', 'NODE_CONNECTED', {nodeId: node.id});
    ...
    onNodeDisconnected (node) {
        ...
        this.notifications.notify('info', 'NODE_DISCONNECTED', {nodeId: node.id});
    ...
    onNodeUpdateRates (node, rates) {
        ...
        this.notifications.notify('info', 'NODE_RATES_UPDATED', {nodeId: node.id});
    ...
    onNodeConnectionError (node) {
        ...
        this.notifications.notify('error', node.error.code, {nodeId: node.id, error: node.error});
    }
