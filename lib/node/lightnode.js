...
class LightNode extends EventEmitter {
    ...
    async start (options = this.options) {
        if (!this.updateTimerOn) {
            ...
            this.mailer.setup(options.mail);
...
//        tickers = tickers.filter(ticker => ticker.error == undefined);
        ...
        if (tickers.length < this.options.minimumExchangesLimit) {
            this.onError('NODE_TICKER_MINIMUM_LIMIT', err);
        ...
        if (!res.err) {
            ...
            this.onError('NODE_TICKER_DELTA_OVERFLOW', err); // 'Node error: delta > deltaMax', 
        }
