...
function getDisconnect(req, res, next) {
    ...
    connection.node.onClientDisconnected(connection);
...
function getNodeTickets(req, res, next) {
    logger.info('getNodeTickets success');

    var connection = getConnection(req, res, next);
    if (!connection)
        return;

    var tickets = connection.server.getNode().getTickets();
