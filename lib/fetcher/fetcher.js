...
//  {"eth_usd":{"high":1447.36274,"low":1359.23432,"avg":1403.29853,"vol":5732433.69952,"vol_cur":4095.29444,"last":1372.6855,"buy":1374.36,"sell":1371.02,"updated":1515951773}}
    ...
    async fetchBitfinex (exchange) {
        return await fetchExchange(exchange, `https://api.bitfinex.com/v1/pubticker/${exchange.options.currency}`, 'last_price');
    }

    async fetchBitstamp (exchange) {
        return await fetchExchange(`https://www.bitstamp.net/api/v2/ticker/${exchange.options.currency}`, 'price');
    }

    async fetchGdax (exchange) {
        return await fetchExchange(`https://api.gdax.com/products/${exchange.options.currency}/ticker`, 'price');
    }

    async fetchGemini (exchange) {
        return await fetchExchange(`https://api.gemini.com/v1/pubticker/${exchange.options.currency}`, 'last');
    }

    async fetchKraken (exchange) {
        return await fetchExchange(`https://api.kraken.com/0/public/Ticker?pair=${exchange.options.currency}`, 'result.XETHZUSD.c.0');
    }

    async fetchPoloniex (exchange) {
        return await fetchExchange(`https://poloniex.com/public?command=returnTicker`, `.${exchange.options.currency}.last`);
    }

    async fetchWex (exchange) {
        return await fetchExchange(`https://wex.nz/api/3/ticker/${exchange.options.currency}`, `.${exchange.options.currency}.last`);
    }
    ...
    async fetchExchange (exchange, url, field) {
        ...
        exchange.lastest = n
        exchange.updateTime = new Date();
        exchange.timestamp = result.timestamp || result.time || result.updated;
    ...
    loadExchanges (options) {
        ...
        for (name in options) {
            ...
            if (!fetcher) {
                this.exchanges.push({name, err: 'FETCHER_BAD_CONFIG', enabled: false});
                continue;
            }

            this.exchanges.push({name, fetcher, options: options[name], enabled: true});
        }
    }
    ...
    async fetchAll () {
        var result = await Promise.all(this.exchanges.map(async (exchange)=>{
            if (exchange.enabled) {
                ...
                await exchange.fether(exchange);
            }
            else {
